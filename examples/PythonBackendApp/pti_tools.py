...
def get_signature(client_id, private_key, url, data=None, content_type="application/json", date=get_http_gmt()):
    method = "GET"
    if data:
        method = "POST"
    ...
    logging.debug(f'Signature (JWS): {signature}')

    return signature

def make_signed_request(client_id, key, url, method, data=None, content_type="application/json"):
    date = get_http_gmt()
    signature = get_signature(client_id, key, url, data, content_type, date)
    if method == "POST":
        resp = requests.post(url, data = data.encode('UTF-8'), headers={"Content-Type": content_type, "Date": date, "x-pti-signature": signature, "x-pti-client-id": client_id})
    else:
        resp = requests.get(url, headers={"Date": date, "x-pti-signature": signature,  "x-pti-client-id": client_id})
